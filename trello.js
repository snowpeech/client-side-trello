...
function addLane(){
    ...
    if (laneName){
        ...
        lane.setAttribute("laneID",laneID);
        lane.innerHTML = `<div class ="listTitle">${laneName}</div>`+ `<button onclick="deleteLane(this, ${laneID})"><i class="far fa-trash-alt"></i></button>`+ `<button id="" onclick="moveLeft(event)" ><i class="fas fa-caret-left"></i></button>
...
function addCard(laneID) {
    ...
    title.innerHTML = `<div id="title-${cardID}">${txtTitle}</div>` + `<button onclick="editTxt(event, ${cardID})"><i class="fas fa-pen"></i></button> <button onclick="deleteCard(this, ${cardID})"><i class="far fa-trash-alt"></i></button><button id="" onclick="moveLeft(event,this)" ><i class="fas fa-caret-left"></i></button> <button onclick="moveUp()" ><i class="fas fa-caret-up"></i></button><button onclick="moveDown()" ><i class="fas fa-caret-down"></i></button><button id="" onclick="moveRight()" ><i class="fas fa-caret-right"></i></button>`;        
...
function editTxt(e,cardID){
    ...
    if (editCheck){
        //do nothing - edit field already exists
    }
    else {
 ...
 function grabText(t,cardID){
    let newWords = document.getElementById(`editting-${cardID}`).value;
